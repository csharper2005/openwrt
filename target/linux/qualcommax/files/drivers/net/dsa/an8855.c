...
static u32 en8855_get_phy_flags(struct dsa_switch *ds, int port)
{
	...
	dev_err(priv->dev, "Calibration is required.\n");
				      ...
				      const struct phylink_link_state *state)
{
	...
	dev_err(priv->dev, "Config CPU port.\n");
...
static int an8855_read_switch_id(struct an8855_priv *priv)
{
	...
	dev_err(priv->dev, "Switch id %x detected\n", id);
	if (ret)
		return ret;
...
static int an8855_switch_probe(struct platform_device *pdev)
{
	...
	dev_err(priv->dev, "Switch reset\n");

		/* AN8855H */

		regmap_read(priv->regmap, AN8855_PKG_SEL, &val);
		dev_err(priv->dev, "PKG_SEL: %x, PAG_SEL_AN8855H: %x\n", (val & 0x7), AN8855_PAG_SEL_AN8855H);


		/* Release global PHY power down */
		regmap_write(priv->regmap, AN8855_RG_GPHY_AFE_PWD, 0x0);

		dev_err(priv->dev, "Set MCU NOP CMD\n");
		/* MCU NOP CMD */
		regmap_write(priv->regmap, AN8855_RG_GDMP_RAM, 0x846);
		regmap_write(priv->regmap, AN8855_RG_GDMP_RAM + 4, 0x4a);
		/* regmap_set_bits(priv->regmap, AN8855_RG_GDMP_RAM, 0x846); */
		/* regmap_set_bits(priv->regmap, AN8855_RG_GDMP_RAM + 4, 0x4a); */

		dev_err(priv->dev, "Enable MCU\n");
		/* Enable MCU */
		regmap_read(priv->regmap, AN8855_RG_CLK_CPU_ICG, &val);
		regmap_write(priv->regmap, AN8855_RG_CLK_CPU_ICG, val | AN8855_MCU_ENABLE);
		/* regmap_set_bits(priv->regmap, AN8855_RG_CLK_CPU_ICG, val | AN8855_MCU_ENABLE); */
		usleep_range(1000, 5000);

		dev_err(priv->dev, "Disable MCU watchdog\n");
		/* Disable MCU watchdog */
		regmap_read(priv->regmap, AN8855_RG_TIMER_CTL, &val);
		regmap_write(priv->regmap, AN8855_RG_TIMER_CTL, (val & (~AN8855_WDOG_ENABLE)));
		/* regmap_set_bits(priv->regmap, AN8855_RG_TIMER_CTL, (val & (~AN8855_WDOG_ENABLE)));; */

		/* Configure interrupt */
		regmap_write(priv->regmap, AN8855_RG_INTB_MODE, (0x1 << 0x0));

		dev_err(priv->dev, "Set some regs\n");
		regmap_write(priv->regmap, 0x10207e04, 0xa08600e5);
		regmap_write(priv->regmap, 0x10207e08, 0x816727);
